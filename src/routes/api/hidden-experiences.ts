...
export const Route = createFileRoute("/api/hidden-experiences")({
  server: { handlers: { GET: async ({ request }) => {
    ...
    try {
      const upstream = await fetch(`${BACKEND}/chat/experiences-hidden?location=${encodeURIComponent(location)}&query=${encodeURIComponent(query)}&limit=${encodeURIComponent(limit)}`, { headers: { Accept: "application/json" } });
    ...
    } catch (error) { console.error("Hidden Places upstream error:", error); return Response.json({ error: "Hidden Places service is temporarily unavailable." }, { status: 502 }); }
  }}}
});
