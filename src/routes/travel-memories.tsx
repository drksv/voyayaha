...
function TravelMemoriesPage() {
  ...
  return <div className="mx-auto max-w-7xl px-5 py-12 sm:px-8">
    <nav className="text-xs text-muted-foreground" aria-label="Breadcrumb"><Link to="/" className="hover:text-foreground">Home</Link><span className="mx-2">/</span><span className="text-foreground">Travel Memories</span></nav>
    <header className="mt-8 max-w-4xl"><p className="eyebrow text-sunrise">Your journeys. Your memories.</p><h1 className="mt-3 text-4xl leading-tight sm:text-6xl">Travel Memories</h1><p className="mt-5 text-lg leading-relaxed text-foreground/75">See where the Voyayaha community has travelled — and add a memory of your own. Each approved memory becomes a pin with its photo and story.</p></header>
    {loading && <p className="mt-8 text-sm text-muted-foreground">Loading memories…</p>}{error && <p className="mt-6 rounded-sm border border-border bg-secondary/40 p-4 text-sm">{error}</p>}{usingDemo && !error && <p className="mt-6 rounded-sm border border-border bg-secondary/40 p-4 text-sm">Map preview mode. Add <code>VITE_WORDPRESS_API_BASE_URL</code> in Cloudflare and install the included WordPress API plugin to load and submit real memories.</p>}
